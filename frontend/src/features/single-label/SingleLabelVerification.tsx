...
function fieldLabel(field: CanonicalLabelField): string {
  return FIELD_CONFIGS.find((config) => config.name === field)?.label ?? field;
}

function resultOrder(result: FieldResult): number {
  return FIELD_CONFIGS.findIndex((config) => config.name === result.field);
}

function formatVerdict(verdict: VerificationResult["overall_verdict"]): string {
